...
for i in range(len(Runs)):
    fname=str(tau[Runs[i]])+'daynokpp/'+Year+'Psi.nc' 
    psi,T=RMOC(fname,Y)
    Psianom=abs(psi)-abs(psi1)
    cf=plt.contourf(y,Rho,Psianom,Psi_levs,cmap=plt.cm.seismic)
    cbar = plt.colorbar(cf, ticks=Psi_ticks, shrink=0.8)
    plt.clim(-1,1)
    plt.title(str(tau[Runs[i]])+"day Remapped RMOC absolute annomaly")
    plt.xlabel('Distance (km)')
    plt.ylabel('Temp')
    cbar.ax.set_ylabel('$\psi_{res}$ anomally (Sv)')
    x=( os.path.expanduser('~')+"/Figures/")
    plt.savefig(x+str(tau[Runs[i]])+'daynokpp/'+str(Year)+'RMOCTanom(abs)'+str(tau[Runs[i]])+".png")
    plt.clf() 
